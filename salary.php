...
<html><head>
   ...
   background-size:cover;font-family: sans-serif;border:none;margin: none;">
         <form method="post" action="salary.php" style="margin: 0;">
    <ul>
    <li><a>Account</a>
        <ul>
...
<a href="salary.php"><input type="submit" name="submit" class="submit" value="search" style="width: 75px; height: 28px;"></a>
...
// REGISTER USER
    if (!isset($_POST['submit'])) {
     $query1 = "SELECT  userid,sum(totalprice) as totalprice from stotalprice group by userid";
  	$results1 = mysqli_query($db, $query1);
  	if (mysqli_num_rows($results1) > 0) {
  	  while( $row = mysqli_fetch_assoc($results1)){
  	  echo "<tr><td>".$row["userid"]."</td><td>".$row["totalprice"]."</td></tr>";
  	}
    echo "</table>";
}
else
...
if (isset($_POST['submit'])) {
  ...
  $search = mysqli_real_escape_string($db, $_POST['usearch']);
    $query = "SELECT  userid,totalprice from stotalprice where userid='$search'  group by userid ";
