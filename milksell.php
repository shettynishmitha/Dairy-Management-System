...
<html><head>
   ...
   background-size:cover;font-family: sans-serif;border:none;margin: none;">
         <form method="post" action="milksell.php" style="margin: 0;">
    <ul>
    <li><a>Account</a>
        <ul>
...
// REGISTER USER
    if (!isset($_POST['submit'])) {
     $query1 = "select * from stotalprice";
...
echo '<td><a href="sdelete.php?id='. $row['id'] . '">Delete</td>';
echo "</tr>";
  	}
    echo "</table>";
}
...
if (isset($_POST['submit'])) {
  ...
  $fsearch = mysqli_real_escape_string($db, $_POST['fsearch']);
    ...
    $query = "select * from stotalprice where sdate BETWEEN '$fsearch' AND '$lsearch'";
